...
public class vInput : MonoBehaviour
{
    ...
    public ControllerMode controllerMode = ControllerMode.KeyboardAndMouse;
    ...
    void OnGUI()
    {
        if (controllerMode ==ControllerMode.Auto)
        {
            switch (inputDevice)
            {
        ...
        {
            if (inputDevice == InputDevice.MouseKeyboard && controllerMode == ControllerMode.Joystick)
            {
                inputDevice = InputDevice.Joystick;
            }
            else if (inputDevice == InputDevice.Joystick && controllerMode == ControllerMode.KeyboardAndMouse)
            {
                inputDevice = InputDevice.MouseKeyboard;
            }
        }
...
{
    public GenericInput positiveButton;
    public GenericInput negativeButton;
    
    public GenericAxis (GenericInput positiveButton, GenericInput negativeButton)
    {
        this.positiveButton = positiveButton;
        this.negativeButton = negativeButton;
    }
    
    public GenericAxis (GenericInput positiveButton)
    {
        this.positiveButton = positiveButton;
        this.negativeButton = new GenericInput("","","");
    }
    
    public float GetAxis()
    {
        if (!negativeButton.useInput || positiveButton.isAxis)
            return positiveButton.GetAxis();
        else
        {
            float val = 0;
            if (positiveButton.isAxis)
                val += positiveButton.GetAxis();
            else
                val += positiveButton.GetButton() ? 1 : 0;
            if (negativeButton.isAxis)
                val -= negativeButton.GetAxis();
            else
                val -= negativeButton.GetButton() ? 1 : 0;
            return val;
        }
    }
    ...
    {
        if (!negativeButton.useInput || positiveButton.isAxis)
            return positiveButton.GetAxisRaw();
        else
        {
            float val = 0;
            if (positiveButton.isAxis)
                val += positiveButton.GetAxisRaw();
            else
                val += positiveButton.GetButton() ? 1 : 0;
            if (negativeButton.isAxis)
                val -= negativeButton.GetAxisRaw();
            else
                val -= negativeButton.GetButton() ? 1 : 0;
            return val;
        }
